...
//checks for JIS
int isjis(unsigned char p1, unsigned char p2) { 
    if (p1 >= 0x81 && p1 <= 0x9F || p1 >= 0xE0 && p1 <= 0xFC) {
        if (p2 >= 0x40 && p2 <= 0x7E || p2 >= 0x80 && p2 <= 0xFC) {
            return 1;
        }
    }
    return 0;
...
char *pad_v(char bin[33], int v, char mode[5]){
    ...
    if (v >= 1 && v <= 9){
        if(strcmp(mode, "0001") == 0){
            n = 10;
        } else if (strcmp(mode, "0010") == 0){
            n = 9;
        } else if(strcmp(mode, "0100") == 0){
            n = 8;
        } else {
            n = 8;
        }
    ...
    } else printf("version does not exist");
